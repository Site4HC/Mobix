...
{
    public class MoyoParserService : BaseParserService, IStoreParserService
    {
        public MoyoParserService(IHttpClientFactory httpClientFactory) : base(httpClientFactory)
        {
        }

        public string StoreName => "Moyo";
        ...
        public async Task<ParserResult> FindPriceAsync(Smartphone smartphone, string storeBaseUrl)
        {
            try
            {
                ...
                string searchUrl = $"{storeBaseUrl}ua/search/new/?q={searchTerm}";
                ...
                if (doc == null)
                {
                    ...
                    {
                        IsSuccess = false,
                        ErrorMessage = "Не вдалося завантажити сторінку Moyo"
                    };
                }
                ...
                {
                    var nameNode = node.SelectSingleNode(".//a[contains(@class, 'product-card_title')]");
                    ...
                    var priceNode = node.SelectSingleNode(".//div[contains(@class, 'product-card_price_current')]");
                ...
                {
                    IsSuccess = false,
                    ErrorMessage = $"Релевантний смартфон (ціна > {MIN_SMARTPHONE_PRICE} грн) не знайдено на сторінці пошуку Moyo."
                };
            }
            catch (Exception ex)
            {
